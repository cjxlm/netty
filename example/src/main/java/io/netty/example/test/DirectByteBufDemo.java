...
public class DirectByteBufDemo {
    ...
    public void apiTest() {
        ...
        System.out.println("写入一段内111容后ByteBuf为===========>" + buf.toString());
        ...
        // 5.清空读写指针
        buf.clear();
        System.out.println("将读写指33333针清空后ByteBuf为==========>" + buf.toString());
        //System.out.println("5.ByteBuf中的内容为===============>" + Arrays.toString(buf.array()) + "\n");

        // 6.再次写入一段内容，比第一段内容少
        byte[] bytes2 = {1, 2, 3};
        buf.writeBytes(bytes2);
        System.out.println("写入的bytes3333为====================>" + Arrays.toString(bytes2));
